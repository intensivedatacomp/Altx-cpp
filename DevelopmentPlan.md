...
1. `dev-cpu` and `runtime-cpu` -- everything needed for stages 1 to 4. **Both enabled**:
   `runtime-cpu` joined the matrix with the first `CMakeLists.txt`, since that is the point at
   which it has something to compile. It ships `altx-serial` and `altx-omp`, built with
   `ALTX_ENABLE_TESTS=OFF`: the tests belong to `build-test.yml`, which runs them across the
   preset matrix in `dev-cpu`, and nothing they produce reaches a release image.
...
- **It runs inside `dev-cpu`, at the content hash resolved from the working tree**, not on a bare
  runner. `runtime-cpu` is compiled inside that image, so a job that built anywhere else would be
  testing a toolchain nothing ships with. The hash is computed by `scripts/ci/images.py`, the same
  resolver `docker-images.yml` builds with, which is the "no data flows between jobs" rule applied
  across two workflows. The one case it cannot cover is a pull request that *changes* the
  development image: the two workflows are triggered together and GitHub orders neither, so the
  tag may not exist yet. The job then falls back to `dev-cpu:edge` and says so with a warning
  annotation, rather than failing on a race.
- **`-Werror` is a `-D` flag on the configure line, not a `ci-*` preset.** The plan's
  [Three compiler-flag decisions](#three-compiler-flag-decisions) says the CI presets turn it on;
  in practice that would mean a parallel configure, build and test preset for every configuration
...
`scripts/ci/coverage.sh` is the whole of it, and CI calls that script rather than spelling the
steps out in YAML -- the same "CI is a second consumer" rule the Dockerfiles follow, so a coverage
number can be reproduced locally in one command.

Two details are not obvious and were both found by doing it:

- **Two captures, not one.** A translation unit that is never executed writes no `.gcda`, so it is
  absent from the report entirely -- the least covered code silently leaving the denominator. A
  `--capture --initial` pass reads the `.gcno` files, giving every instrumented line a zero, and
  the merge of the two is the honest number. The `cli.runs` CTest entry exists for the same
  reason: `apps/` is only covered if something actually executes the binary.
- **lcov 2.0 turns three warnings into errors**, and all three are properties of this project
  rather than defects: `mismatch` (gcov's function end lines disagree with the source for anything
  macro-expanded -- every `TEST()`), `unused` (a `--remove` pattern matching nothing) and `empty`
  (what `--capture --initial` legitimately produces). They are listed in `--ignore-errors` with
  that reasoning next to them.

The badge is generated by `scripts/ci/coverage_badge.py` and **committed** to `.badges/`, as the
Python implementation does, rather than fetched from shields.io: the README of a repository that
may be private then renders without an external request. The percentage is computed from the
tracefile's `LH`/`LF` records rather than by parsing `lcov --summary`, whose output is meant for a
terminal -- and whose `--list` and `--summary` already disagree on how to present it.
